...
const GENERATED_TS_ENTRIES = ["index", "child-auth-bootstrap", "child-communication-bridge"];
const CHILD_AUTH_SOURCE = "src/child-auth-bootstrap.ts";
...
export async function buildRuntime({
	...
	try {
		const result = await build({
			...
			entryPoints: {
				index: "src/index.ts",
				"child-auth-bootstrap": CHILD_AUTH_SOURCE,
...
export function validateEagerGraph(metadata) {
	...
	const childEntries = [
		[CHILD_AUTH_SOURCE, findEntry(outputs, CHILD_AUTH_SOURCE)],
		[CHILD_BRIDGE_SOURCE, findEntry(outputs, CHILD_BRIDGE_SOURCE)],
	];
	...
	for (const [source, entry] of childEntries) {
		if ([...eagerInputs].some((input) => input.endsWith(`/${source}`))) {
			throw new Error(`Child-process entry is eager: ${source}`);
		}
		if (eagerOutputs.has(entry)) {
			throw new Error(`Child-process entry is reachable from the main extension entry: ${source}`);
		}
	}
...
export async function validateGeneratedFiles(outputDirectory) {
	...
	if (!mainSource.includes('"./child-auth-bootstrap.ts"')) {
		throw new Error("Generated main entry does not reference the generated child auth bootstrap");
	}
